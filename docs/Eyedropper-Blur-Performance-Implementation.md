...
Last Updated: 09-12-26 10:14 · Revision 1.10
...
**Display checks owed**, since every render check in this work is a pixel test on the offscreen platform. This work adds three: the loupe at each of the four sample sizes over a screenshot, the loupe near the right and top edges of the viewport where it flips, and the Eyedropper's bar at a narrow window, which is now the widest bar in the application. They join what is left after the run Doug made on 09-12-26, recorded in each work's own notes: the freeform blur work still owes a painted blur region and an erased one with the brush cursor at two sizes, a Whole Layer region with each source mode, and a freeform highlight with its point handles; the Basic Shape remainder work still owes each blur mode over a screenshot; the Vector Item Properties work still owes a highlight in Multiply over dark text; and the General UI notes' Section 19.2 still awaits the right-click half of row 20. Everything else those works owed passes. Two findings from that run bear on this work: the Blur tool's Tool Options Bar appears to overflow behind Qt's extension button at Doug's window width, which the Eyedropper's bar, now the widest in the application, will meet sooner; and Cinnamon's window manager takes Alt plus the left button as its window-move gesture (`org.cinnamon.desktop.wm.preferences mouse-button-modifier` reads `<Alt>`, verified 09-12-26), so no Alt+drag reaches the canvas on that desktop — which leaves the blur eraser of Blur PRD 2.8 with no working route, where the Zoom tool was given right-click as a second one.
