...
**Next required step:** Phase 7, Tool themes and presets, in a new session pasting `docs/General-UI-Implementation-Kickoff-Prompt.md`: the preset dropdown, the Tool Themes dialog, the Manage Presets dialog, JSON storage in the application data directory, the `.smktheme` import and export, and the Active Theme label. No kickoff decision is scheduled for Phase 7; the welcome-panel card (kickoff decision 5) waits for Phase 8.
